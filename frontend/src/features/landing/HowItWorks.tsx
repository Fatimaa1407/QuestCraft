  ...
  return (
    <section id="how" className="py-24 sm:py-28">
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        ...
        >
          <span className="font-mono text-xs uppercase tracking-[0.14em] text-cyan-600 dark:text-cyan-400">{t('landing.how.eyebrow')}</span>
          ...
          <p className="mt-3.5 text-[15.5px] leading-relaxed text-slate-600 dark:text-slate-400">{t('landing.how.subtitle')}</p>
        </motion.div>
        ...
        >
          <div className="pointer-events-none absolute inset-x-16 top-[43px] hidden h-px overflow-hidden bg-slate-900/[0.08] lg:block dark:bg-white/[0.08]">
            <motion.div
              initial={{ scaleX: 0 }}
              animate={{ scaleX: 1 }}
          ...
          {steps.map((step, index) => (
            <motion.div key={step.titleKey} variants={cascadeItem} className="relative z-10">
              <GlassCard className="p-6">
                ...
                <span className="mt-4 block font-mono text-[11.5px] tracking-[0.1em] text-cyan-600 dark:text-cyan-400">
                  {t('landing.how.stepLabel')} 0{index + 1}
                </span>
                <h3 className="mt-2 text-[15.5px] font-bold text-slate-900 dark:text-white">{t(step.titleKey)}</h3>
                <p className="mt-2 text-[13px] leading-relaxed text-slate-600 dark:text-slate-400">{t(step.descKey)}</p>
              </GlassCard>
            </motion.div>
          ))}
        </motion.div>
