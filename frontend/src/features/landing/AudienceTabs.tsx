  ...
  return (
    <section id="audience" className="py-24 sm:py-28">
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        ...
        >
          <span className="font-mono text-xs uppercase tracking-[0.14em] text-cyan-600 dark:text-cyan-400">{t('landing.audience.eyebrow')}</span>
        ...
        <motion.div
          ...
          className="mt-8 inline-flex gap-1 rounded-full border border-slate-900/[0.08] bg-slate-900/[0.03] p-1 dark:border-white/[0.08] dark:bg-white/[0.03]"
          role="tablist"
        >
          <motion.button
            type="button"
            ...
            className={`rounded-full px-4.5 py-2 text-[13.5px] font-semibold transition-colors ${
              isStudent
                ? 'bg-gradient-to-r from-blue-600 to-cyan-500 text-[#051019]'
                : 'text-slate-600 hover:bg-slate-900/[0.06] hover:text-slate-900 dark:text-slate-400 dark:hover:bg-white/[0.06] dark:hover:text-white'
            }`}
          >
        ...
        >
          <div>
            ...
            <ul className="mt-4 flex flex-col gap-3.5">
              {bullets.map((key) => (
                <li key={key} className="flex items-start gap-2.5 text-[14.5px] leading-relaxed text-slate-600 dark:text-slate-400">
                  <Check size={18} className="mt-0.5 shrink-0 text-emerald-500 dark:text-emerald-400" />
                  {t(key)}
                </li>
              ))}
            </ul>
          ...
          <GlassCard hoverLift={false} className="p-7">
            {stats.map(([labelKey, value]) => (
              <div key={labelKey} className="flex items-center justify-between border-b border-slate-900/[0.08] py-3 text-[13px] last:border-0 dark:border-white/[0.08]">
                <span className="text-slate-600 dark:text-slate-400">{t(labelKey)}</span>
                <span className="font-mono text-cyan-600 dark:text-cyan-400">{value}</span>
              </div>
            ))}
          </GlassCard>
        </motion.div>
